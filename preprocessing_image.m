...
% PREPROCESSING_IMAGE MATLAB code for preprocessing_image.fig
%      PREPROCESSING_IMAGE, by itself, creates a new PREPROCESSING_IMAGE or raises the existing
%      singleton*.
%
%      H = PREPROCESSING_IMAGE returns the handle to a new PREPROCESSING_IMAGE or the handle to
%      the existing singleton*.
%
%      PREPROCESSING_IMAGE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PREPROCESSING_IMAGE.M with the given input arguments.
%
%      PREPROCESSING_IMAGE('Property','Value',...) creates a new PREPROCESSING_IMAGE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before preprocessing_image_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to preprocessing_image_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help preprocessing_image

% Last Modified by GUIDE v2.5 03-Mar-2018 11:18:43
